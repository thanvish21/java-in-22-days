/* api/grade.js — Vercel Node serverless function for server-side grading of Java problems.

   Same-origin endpoint POST /api/grade. The browser cannot run Java, so all grading is
   here. The frontend (js/runner.js -> HFTRunner.grade) posts
   { language:"java", code, runtimeFlags?, tests:[{name,stdin,expectStdout,matchMode,
...
module.exports = async (req, res) => {
  ...
  // Default to Java: this is the Java platform.
  ...
  try {
    ...
    for (const t of tests) {
      ...
      let r;
      try {
        r = await runViaJudge0(lang, code, opts);
      } catch (err) {
